...
public class MultiplayerGameScreen implements Screen, ChatListener, GameEventListener {
    private Player player;
    private List<String> remotePlayers = new ArrayList<>();
    CompositeMapSystem compositeMapSystem;
    ...
    public void show() {
        if (!initialized) {
            try {
                ...
                Main.getMain().getGameClient().sendPlayerNewPosition(startX, startY, "Farm");
...
// اگر رابط کاربری یخچال باز است
    ...
    public void handleGameEvent(GameEvent event) {
        ...
        switch (type) {
            case UPDATE:
                updatePlayers(event.getGameData());
                break;

            case UPDATE_POSITIONS:
                updatePlayersPosition(event.getGameData());
                break;
        }
    }
