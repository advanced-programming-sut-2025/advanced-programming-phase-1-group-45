...
public class ForagingInventoryWindow extends Window {
    private final Table contentTable;
    private final Skin skin;
    private final TextButton closeButton;
    private final int COLUMNS = 5;
    private SeedSelectionListener selectionListener;
    private ForagingItem selectedSeed;
    ...
    private Table createSeedCell(ForagingItem seed, float iconSize) {
...
//    private void selectSeed(ForagingItem seed) {
//        selectedSeed = seed;
//
...
//    }
    ...
    public ForagingItem getSelectedSeed() {
        return selectedSeed;
    }

    private Array<ForagingItem> getPlayerSeeds() {
        Array<ForagingItem> seeds = new Array<>();
        for (InventoryItem item : InventoryManager.getInstance().getPlayerInventory().getItems().values()) {
            if (item instanceof ForagingItem && item.getQuantity() > 0) {
                seeds.add((ForagingItem) item);
            }
        }
