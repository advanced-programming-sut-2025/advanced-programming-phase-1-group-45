...
public class CraftManager {
    ...
    public boolean craftItem(String recipeName, User user) {
        if (!learnedRecipes.contains(recipeName)) {
            System.out.println("Recipe not learned: " + recipeName);
            return false;
        }
        ...
        if (skillLevel < recipe.getRequiredSkillLevel()) {
            System.out.println("Required skill level not met. You need " +
                    recipe.getSkillType() + " level " +
        ...
        if (!recipe.canCraft(inventory)) {
            System.out.println("Missing required materials!");
            return false;
        }

        int energyCost = recipe.getEnergyCost();
        System.out.println("Energy required for crafting: " + energyCost);

        if (!user.getPlayer().consumeEnergy(energyCost)) {
            System.out.println("Not enough energy to craft this item!");
            return false;
        }
        ...
        for (Map.Entry<String, Integer> entry : recipe.getIngredients().entrySet()) {
            ...
            System.out.println("Used " + quantity + " " + item);
        }

        user.addItem(recipe.getName(), 1);
        System.out.println("Crafted 1 " + recipe.getName());

        increaseSkillExperience(user, recipe.getSkillType(), 5);
        System.out.println("Gained 5 XP in " + recipe.getSkillType());
    ...
    private Map<String, Integer> getUserInventory(User user) {
        ...
        // برای فعلاً یک نمونه خالی برمی‌گردانیم
        return inventory;
    }
}
